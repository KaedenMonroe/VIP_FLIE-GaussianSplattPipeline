...
class MetashapeProject:
    ...
    def project_execute(self) -> None:
        ...
        self.save_project(doc, self.metashape_dir, self.project_name)
        self.add_photos_to_chunk(chunk, self.images)
        self.save_project(doc)
        self.align_photos(chunk)
        self.save_project(doc)
        self.export_cameras_to_colmap(chunk, export_path, self.project_name)
        photocount = len(self.images)
        self.display_metrics(chunk, photocount, self.metashape_dir, self.project_name)
        self.save_project(doc)
        Metashape.app.quit()
...
# Initialize the parser
    ...
    project.project_execute(export_path, name, photos, metashape_directory)